...
WINDOW_SIZE = 200
STEP_SIZE = 25
DATA_PATH = os.path.join('data', 'A_DeviceMotion_data')
N_SPLITS = 5
EPOCHS = 20
BATCH_SIZE = 64

USE_FFT = True  # <-- Enable FFT-only mode
...
def apply_fft(window):
    fft_transformed = np.fft.fft(window, axis=0)
    fft_magnitude = np.abs(fft_transformed)[:len(fft_transformed) // 2]
    return fft_magnitude

def segment_data_fft(df, window_size, step_size):
    segments = []
    labels = []
    for i in range(0, len(df) - window_size, step_size):
        ...
        data_fft = apply_fft(data)
        segments.append(data_fft)
        labels.append(label)
...
if __name__ == '__main__':
    ...
    print("Segmenting with FFT (no scaling)...")
    segments, labels = segment_data_fft(df, WINDOW_SIZE, STEP_SIZE)
    ...
    for fold, (train_idx, test_idx) in enumerate(skf):
        ...
        model = create_model(input_shape=(X_train.shape[1], X_train.shape[2]), num_classes=categorical_labels.shape[1])
